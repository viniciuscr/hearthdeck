...
impl<'a, Message: Clone + 'static> ApplicationButton<'a, Message> {
    ...
    pub fn new(
        ...
        let max_name_len = 34 - source_suffix_len;
        ...
        .drag_content(move || AppletString(path_.as_ref().cloned().unwrap_or_default()))
        .on_start(on_start)
        .on_cancel(on_cancel)
        .on_finish(on_finish)
        .into();
    ...
    Message: Clone,
{
    ...
    fn draw(
        ...
        // Skip tiles outside the visible region: the scrollable does not cull
        // its children, so drawing every tile on every frame dominates the
        // frame cost once the artwork fills the tile.
