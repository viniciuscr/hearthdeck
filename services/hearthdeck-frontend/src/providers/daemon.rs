...
/// The owner a feature's collections come back with. A collection under this
/// owner was created and is regenerated by the categorization model, so a client
/// shows it while the feature is on and the daemon removes it when it is off.
pub const MODEL_OWNER: &str = "laya";

/// The rail the model's scan can put on the dashboard: things there are to
/// watch. The daemon names the collection after it, and this is the client
